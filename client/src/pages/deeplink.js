...
export default function App () {
  ...
  useEffect(() => {
    const fetchResources = async () => {
      try {
        const resources = await ky.get('/lti/resources', { credentials: 'include', headers: { Authorization: 'Bearer ' + getLtik() } }).json()
  ...
  const submit = async () => {
    try {
      ...
      const form = await ky.post('/lti/deeplink', { credentials: 'include', json: dataset[resource], headers: { Authorization: 'Bearer ' + getLtik() } }).text()
      $('body').append(form)
    } catch (err) {
      console.log(err)
