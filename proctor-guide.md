...
1. **Smile and Greet Every Student Promptly**: When a student approaches the desk, acknowledge them immediately. A warm _"Hi there! Good morning / afternoon"_ eases tension.
...
- **Required Action**:
  - Remind the student: _"Please power off your phone and smartwatch, and place your backpack and jacket into a cubby."_
...
Ask: _"May I please see your student ID card?"_
...
- _(Manual Fallback)_: If the magnetic stripe is damaged, type their 9-digit Student ID into the box and press **Enter**.
...
- **Action**:
  ...
  3. Clearly state their seat: _"You are at Workstation Seat #12. Please have a seat, wake the computer, and begin your exam. Good luck!"_
...
- **Screen Display**: Amber alert stating: _"Reservation starts at [Time]. Check-in opens at [Time] (5 min prior to slot)."_
- **Action**:
  - Politely inform the student:
    > _"Hi [Name], your exam isn't scheduled to begin until [Time]. Because workstations are currently occupied by ongoing test takers, check-in opens 5 minutes before your slot at [Open Time]. Please wait in the lobby area and come back at that time."_
...
- **Action**:
  - Explain calmly and constructively:
    > _"Unfortunately, because it is more than 15 minutes past your scheduled start time, the testing window for this reservation has expired and the seat has been released. You will need to visit your student dashboard to reschedule for the next available slot."_
  - Return their ID card. Click **Clear**.
  - _(Exception)_: Only if you have received direct, written instruction from the course instructor or CBTF manager to accommodate this specific student, click **Override & Check In**.
...
4. **Return Physical ID**:
   - Hand their ID card back to the student:
     > _"You are all set and checked out! Have a wonderful day."_
...
```
                   ...
                   └─────────────────┬─────────────────┘
                                     ...
                                     ▼
                   ┌───────────────────────────────────┐
                   │  4. LOG NOTE IN CONSOLE & LET     │
                   │     SYSTEM NOTIFY ON CHECKOUT     │
                   │  Use the popup form to record     │
                   │  observations & photo indicator.  │
                   └───────────────────────────────────┘
```

### Logging Incident Notes in the Testing Center Console

Rather than drafting manual emails from scratch, use the console's built-in incident note system:

1. **Three Ways to Open the Note Form**:
   - **From the Header**: Click the **Add Incident / Note** button (`i-lucide-file-warning`) at any time. You can select the student either by their **Workstation Seat Number (#1–48)** or by **Student Name**.
   - **During Check-In**: Click **Add Note** directly on the student verification card if an irregularity occurs at the desk.
   - **During the Exam**: On the **Currently Seated** roster, click the **Note** button in the student's row. If notes already exist, the button highlights with a badge (e.g., `Notes (1)`).
2. **Filling Out the Note**:
   - **Observation Details**: Enter factual, objective descriptions (e.g., _"Student was observed with an index card containing handwritten formulas placed beneath their mousepad at 14:22"_).
   - **Photo Indicator**: If you captured photo evidence on your personal mobile phone, check:
     `[x] Picture(s) taken on proctor device`.
   - Click **Save Note**. Multiple notes can be added for the same student throughout check-in, active testing, and check-out.
3. **Automated Email Notification on Checkout**:
   - You do **not** need to compose or send an email manually.
   - When the student finishes their exam and you execute their check-out at the desk, the system automatically compiles all session notes, timestamps, workstation seat number, and student details.
   - The report is immediately emailed to both the **Course Instructor** and the **CBTF Facility Manager**.
   - If you flagged that picture(s) were taken, the email displays a prominent alert instructing the faculty and manager to reach out to you directly to obtain the photo evidence.
...
1. Remain calm and reassure the student: _"Don't worry, your work is saved in the testing system."_
...
| **Return ID**       | Hand ID card back to student                            | Confirm checkout prompt: _"RETURN student ID"_                       |
...
_Document maintained by CBTF Facility Administration._  
_For questions, facility emergencies, or policy clarifications, contact your lead CBTF Manager._
