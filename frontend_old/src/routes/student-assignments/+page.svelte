<script>
	...
	onMount(async () => {
		try {
			...
			const apiEndpoint = `${env.PUBLIC_BACKEND_URL}/egp-broker-service/api/v1/courses/${courseId}/assignments`;
	...
	async function fetchFreePasses() {
		...
		try {
			...
			const response = await fetch(
				`${env.PUBLIC_BACKEND_URL}/egp-broker-service/api/freepassPool/${$selectedAssignment.courseOfferingId._id}?tags=${tags}`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token.access_token}`
	...
	async function use(assignmentId, passValue) {
		...
		try {
			...
			const response = await fetch(
				`${env.PUBLIC_BACKEND_URL}/egp-broker-service/api/use-pass/${assignmentId}/${passValue}`,
				{
					method: 'POST',
					headers: {
						Authorization: `Bearer ${token.access_token}`
			...
			<tbody>
				{#each assignments as assignment (assignment._id)}
					<tr>
						...
						<td>
							<button class="btn btn-primary" on:click={() => openModal(assignment)}
								>Use FreePass</button
							>
						</td>
					</tr>
									...
									<tr>
										...
										<td>
											<button
												...
												on:click={() => use($selectedAssignment._id, pass.value)}
												>Use
											</button>
										</td>
									</tr>
