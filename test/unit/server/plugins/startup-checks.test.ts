...
describe('startup-checks plugin', () => {
  ...
  it('logs an error in production when required email config keys are empty', async () => {
    const originalEnv = process.env.NODE_ENV
    ...
    expect(spy).toHaveBeenCalledWith(expect.stringContaining('NUXT_EMAIL_FROM'))

    process.env.NODE_ENV = originalEnv
  })

  it('does NOT log errors when user and pass are omitted for whitelisted relay', async () => {
    const originalEnv = process.env.NODE_ENV
    process.env.NODE_ENV = 'production'
  ...
  it('does NOT log errors when all email fields including credentials are present', async () => {
    const originalEnv = process.env.NODE_ENV
    process.env.NODE_ENV = 'production'
