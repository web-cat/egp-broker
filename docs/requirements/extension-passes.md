...
Pass types that have extensionOnly set to true can only be used to "extend a deadline". This means that a pass with a duration of 24 hours that is an extension pass will effectively advance the until date (the late submission cutoff, `acceptUntil`) by 24 hours, while leaving the assignment due date (`dueDate`) unchanged.

In other words, an extension pass does not "reopen" an assignment from the time it is
redeemed until 24 hours later. Instead, it extends the late submission period from the current cutoff deadline to 24 hours
later than the current cutoff deadline.

Example: assignment due on 3/10 at 11:59pm with an acceptUntil date of 3/10 at 11:59pm. The student has one extension pass with a duration
of 24 hours. The student redeems the pass the next morning on 3/11 at 8:00am. The assignment's due date remains 3/10 11:59pm, and the new effective
until deadline for this student is now 3/10 11:59pm + 24 hours == 3/11 11:59pm.

Also, when multiple extension passes are available to use on the same assignment, they
stack contiguously. For example, the first extension pass with a 24 hour duration would move
the until date from 3/10 11:59pm to 3/11 11:59pm. If the student redeems a second extension pass
on that assignment, the until date would move from 3/11 11:59pm to 3/12 11:59pm.
...
passes to extend the until deadline 24 \* 2 hours to 4/7 11:59pm.
