<script>
		...
		try {
			...
			if (course.courseOfferingId._id) {
				...
				const response = await fetch(
						`http://localhost:3000/api/instructor/${course.courseOfferingId._id}/requests`,
						{
							method: 'GET',
							headers: {
	...
	async function grant(id, count) {
		try {
			...
			const response = await fetch(
					`http://localhost:3000/api/instructor/grant-pass/${id}/${count}`,
					{
						method: 'POST',
						headers: {
							Authorization: `Bearer ${token.access_token}`
	...
	async function reject(id) {
		try {
			...
			const response = await fetch(
					`http://localhost:3000/api/instructor/reject-pass/${id}`,
					{
						method: 'POST',
						headers: {
							Authorization: `Bearer ${token.access_token}`
