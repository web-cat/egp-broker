<script>
	...
	async function handleSubmit(event) {
		...
		try {
			const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/egp-broker-service/api/pass-types`, {
				method: 'POST',
				headers: {
	...
	async function generatePasses() {
		try {
			...
			const response = await fetch(
				`${import.meta.env.VITE_BACKEND_URL}/egp-broker-service/api/generate-passes/${$course.courseOfferingId._id}`,
				{
					method: 'POST',
					headers: {
						Authorization: `Bearer ${token.access_token}`,
	...
	async function fetchStudents() {
		try {
			...
			const response = await fetch(
				`${import.meta.env.VITE_BACKEND_URL}/egp-broker-service/api/course-offering/${$course?.courseOfferingId._id}/students/`,
				{
					method: 'GET',
					headers: {
						Authorization: `Bearer ${token.access_token}`
	...
	async function assign(id) {
		try {
			...
			const response = await fetch(
				`${import.meta.env.VITE_BACKEND_URL}/egp-broker-service/api/freepass/${id}/assign/${studentId}`,
				{
					method: 'POST',
					headers: {
						Authorization: `Bearer ${token.access_token}`,
